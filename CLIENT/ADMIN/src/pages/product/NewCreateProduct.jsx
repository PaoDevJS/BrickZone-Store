...
const NewCreateProduct = () => {
  ...
  const handleRemoveImgUrl = (name) => {
    setImgUrl(imgUrl.filter((item) => item.name !== name));
  };
  //   thêm mới sản phẩm
  const fetchApiNewCreateProduct = async () => {
    try {
      ...
      const result = await axios.post(linkApiNewCreateProduct, dataForm);
          ...
          <div className="border border-gray-300 outline-none w-full max-h-[300px] mt-3 py-2 px-3 rounded-md grid grid-cols-12 gap-3">
              ...
              (item, index) =>
                item && (
                  ...
                  >
                    <IoClose
                      size={18}
                      onClick={() => handleRemoveImgUrl(item.name)}
            ...
            <div className="w-[85px] h-[85px] border border-gray-300 rounded-md m-auto overflow-hidden col-span-2">
              ...
              <input
                type="file"
                onChange={(valid) =>
                  setImgUrl((p) => [...p, valid.target.files[0]])
                }
                name="imgUrl"
                id="imgUrl"
