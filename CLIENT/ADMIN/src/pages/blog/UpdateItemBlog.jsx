...
const UpdateItemBlog = () => {
  ...
  const linkUrlApiUpdateItemBlog = `http://localhost:3000/api/v1/blog/update-item-blog/${id}`
  const linkUrlApiGetItemBlog =  `http://localhost:3000/api/v1/blog/get-item-blog/${id}`

  useEffect(() => {
    const fetchApiGetItemBlog = async () => {
      try {
        const result = await axios.get(linkUrlApiGetItemBlog)
    ...
    fetchApiGetItemBlog()
  }, [linkUrlApiGetItemBlog])


  const IsUpdateItemBlog = async () => {
    try {
      ...
      const result = await axios.put(linkUrlApiUpdateItemBlog, dataForm)
      navigate("/quan-ly-tin-tuc/danh-sach-tin-tuc")
  ...
  const handleSubmitUpdateItemBlog = (e) => {
    e.preventDefault()
    IsUpdateItemBlog()
  }
  ...
  return (
    <div className="w-full h-full relative overflow-y-scroll Scroller">
      ...
      <form className="w-[60%] m-auto py-5 flex flex-col gap-5">
        <div>
          <label htmlFor="title" className="min-w-[25%] text-[16px] font-[600] text-gray-600">Tiêu đề:</label>
          <input
            type="text"
            name="title"
            id="title"
        ...
        <div>
          <label htmlFor="title" className="min-w-[25%] text-[16px] font-[600] text-gray-600">Mô tả:</label>
          <textarea
            type="text"
            name="desc"
            id="desc"
        ...
        <div>
          <label htmlFor="title" className="min-w-[25%] text-[16px] font-[600] text-gray-600">Nội dung:</label>
          <textarea
            type="text"
            name="content"
            id="content"
          ...
          <div className="border border-gray-300 w-[150px] h-[150px] rounded-md overflow-hidden">
            {imgUrl ? (
              <label htmlFor="imgUrl">
                <img
                  src={
                    preView ? URL.createObjectURL(imgUrl) : `http://localhost:3000/${imgUrl}`
                  }
                  alt=""
            ...
            <input
              type="file"
              onChange={(valid) => (setImgUrl(valid.target.files[0]), setPreView(valid.target.files[0]))}
              name="imgUrl"
              id="imgUrl"
        ...
        <button onClick={handleSubmitUpdateItemBlog} className="bg-RedDrank w-full py-3 rounded-md text-white uppercase text-[18px] font-[600] cursor-pointer mt-7">
          Cập nhật bài viết
        </button>
      </form>
    </div>
